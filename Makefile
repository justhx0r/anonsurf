...
NIMFLAGS := --panics:off -f:on --embedsrc:off --nimMainPrefix:"AnonSurf" -a:on --docCmd:"pass" --tlsEmulation:off  --warnings:off --genScript:off --benchmarkVM:off --profileVM:off --mm:arc   --implicitStatic:on --hints:off --stackTrace:off --lineTrace:off 
